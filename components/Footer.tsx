...
// FIX 2: Removed unused interface 'ColumnData'
...
const NewsletterSection: React.FC = () => {
  ...
  // The 'icon' prop is a component that receives a 'size' prop, so a more specific type is needed.
  // Since the inlined icons are custom SVGs and not LucideIcon components, we use a more generic
  // functional component type that accepts common SVG/React component props like 'size'.
  ...
  const SocialIcon: React.FC<SocialIconProps> = ({ icon: Icon, href }) => (
    <a
      ...
      className="text-gray-400 hover:text-cyan-400 transition-colors duration-200 p-2 rounded-full border border-gray-700 hover:border-cyan-400"
    >
      <Icon size={20} />
    </a>
  );
  ...
  return (
    <div className="lg:col-span-1">
      ...
      {/* Email Input */}
      <div className="relative border-b-2 border-gray-600 focus-within:border-cyan-500 mb-8">
        <input
          type="email"
          placeholder="Enter your email"
          aria-label="Enter your email for newsletter"
          className="w-full bg-transparent text-gray-300 py-3 pr-10 focus:outline-none placeholder-gray-500"
        />
        <button
          type="submit"
      ...
      <div className="relative">
        <select
          ...
          className="appearance-none w-full bg-gray-800 text-gray-300 py-3 px-4 pr-10 rounded-lg border border-gray-700 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all cursor-pointer"
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-gray-900 text-gray-400 pt-16 font-inter">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Top Section: Main Content Grid */}
        <div className="grid grid-cols-4 gap-12 md:grid-cols-2 lg:grid-cols-4 lg:gap-8 pb-16 border-b border-gray-800">
          ...
          {/* Column 2: Contacts */}
          <div className="lg:col-span-1">
            <h4 className="text-lg font-semibold text-white mb-6">Contacts</h4>
            <ul className="space-y-4">
              {contactDetails.map((item, index) => (
                <li key={index} className={`flex items-start text-sm ${item.isAddress ? 'gap-2' : 'gap-4'}`}>
                  {/* The icon is a LucideIcon component */}
